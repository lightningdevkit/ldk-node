...
class NodeSetup:
    def __init__(self, node, node_id, tmp_dir, listening_addresses, stores=None, backup_tmp_dir=None):
        self.node = node
        self.node_id = node_id
        self.tmp_dir = tmp_dir
        self.listening_addresses = listening_addresses
        self.stores = stores  # (primary, ephemeral) or None
        self.backup_tmp_dir = backup_tmp_dir
        
    def cleanup(self):
        self.node.stop()
        time.sleep(1)
        self.tmp_dir.cleanup()
        if self.backup_tmp_dir is not None:
            self.backup_tmp_dir.cleanup()
...
def setup_two_nodes(esplora_endpoint, port_1=2323, port_2=2324, use_tier_store=False) -> tuple[NodeSetup, NodeSetup]:
    ...
    if use_tier_store:
        node_1, stores_1, backup_tmp_dir_1 = setup_node_with_tier_store(
            tmp_dir_1.name,
            esplora_endpoint,
    ...
    if use_tier_store:
        node_2, stores_2, backup_tmp_dir_2 = setup_node_with_tier_store(
            tmp_dir_2.name,
            esplora_endpoint,
            listening_addresses_2,
        )
    else:
        node_2 = setup_node(tmp_dir_2.name, esplora_endpoint, listening_addresses_2)
        stores_2 = None
        backup_tmp_dir_2 = None
...
def setup_node_with_tier_store(tmp_dir, esplora_endpoint, listening_addresses) -> tuple[Node, tuple[TestKvStore, TestKvStore], tempfile.TemporaryDirectory]:
    mnemonic = generate_entropy_mnemonic(None)
    node_entropy = NodeEntropy.from_bip39_mnemonic(mnemonic, None)
    config = default_config()
    ...
    backup_tmp_dir = tempfile.TemporaryDirectory("_ldk_node_backup")
    ...
    builder.set_network(DEFAULT_TEST_NETWORK)
    builder.set_listening_addresses(listening_addresses)
    builder.set_backup_storage_dir_path(backup_tmp_dir.name)
    builder.set_ephemeral_store(ephemeral)
    
    return builder.build_with_store(node_entropy, primary), (primary, ephemeral), backup_tmp_dir
...
class TestLdkNode(unittest.TestCase):
    ...
    def test_tier_store(self):
        ...
        try:
            ...
            primary, ephemeral = setup_1.stores
