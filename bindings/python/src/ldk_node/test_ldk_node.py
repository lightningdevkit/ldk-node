...
def setup_node_with_tier_store(tmp_dir, esplora_endpoint, listening_addresses):
    ...
    builder.set_tier_store_backup(DynStore.from_store(backup))
    builder.set_tier_store_ephemeral(DynStore.from_store(ephemeral))
    
    return builder.build_with_tier_store(node_entropy, DynStore.from_store(primary)), (primary, backup, ephemeral)
