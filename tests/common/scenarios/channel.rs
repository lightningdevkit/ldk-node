...
pub(crate) async fn open_channel_to_external<E: ElectrumApi>(
	...
	let ext_channel_id = ext_channels
		...
		.or_else(|| {
			// Fallback: match by peer_id, but only active channels to avoid
			// picking up closing/closed channels from previous scenarios.
			ext_channels.iter().find(|ch| ch.peer_id == node.node_id() && ch.is_active)
		})
...
pub(crate) async fn open_channel_from_external<E: ElectrumApi>(
	...
	// Ensure peer is connected (may have been disconnected by a previous scenario).
	let _ = peer.connect_peer(node.node_id(), ldk_addr.clone()).await;
...
pub(crate) async fn cooperative_close_by_ldk<E: ElectrumApi>(
	...
	let ext_node_id = peer.get_node_id().await.unwrap();
	// Allow pending HTLCs and monitor updates to settle before initiating close.
...
pub(crate) async fn cooperative_close_by_external<E: ElectrumApi>(
	...
	// Allow the closing transaction to be negotiated and broadcast.
	tokio::time::sleep(Duration::from_secs(2)).await;
	generate_blocks_and_wait(bitcoind, electrs, 1).await;
	node.sync_wallets().unwrap();
...
pub(crate) async fn force_close_by_external<E: ElectrumApi>(
	...
	// Wait for the commitment tx to appear in the mempool before mining.
	// External nodes (especially Eclair) may take 10+ seconds to broadcast
	// the commitment tx after the force-close API returns.
	let mempool_before: Vec<String> =
		bitcoind.call::<Vec<String>>("getrawmempool", &[]).unwrap_or_default();
	for i in 0..30 {
		tokio::time::sleep(Duration::from_secs(1)).await;
		let mempool: Vec<String> =
			bitcoind.call::<Vec<String>>("getrawmempool", &[]).unwrap_or_default();
		if mempool.len() > mempool_before.len() {
			println!(
				"force_close_by_external: commitment tx appeared in mempool after {}s ({} -> {} txs)",
				i + 1,
				mempool_before.len(),
				mempool.len()
			);
		...
		if i == 29 {
			println!(
				"force_close_by_external: WARNING: no new tx in mempool after 30s, mining anyway"
			);
		}
	}

	// Mine blocks to confirm the commitment tx and trigger ChannelClosed.
	generate_blocks_and_wait(bitcoind, electrs, 6).await;
	node.sync_wallets().unwrap();
	tokio::time::sleep(Duration::from_secs(2)).await;
	node.sync_wallets().unwrap();
