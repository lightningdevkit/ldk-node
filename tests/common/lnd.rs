...
impl ExternalNode for TestLndNode {
	...
	async fn open_channel(
		...
		let txid_bytes = match response.funding_txid {
			...
			Some(lnd_grpc_rust::lnrpc::channel_point::FundingTxid::FundingTxidStr(s)) => {
				// LND normally returns FundingTxidBytes; this branch exists for
				// forward-compatibility but has never been observed in practice.
	...
	async fn pay_invoice(&self, invoice: &str) -> Result<String, TestFailure> {
		...
		Err(self.make_error("payment stream ended without terminal status".to_string()))
	}
	...
	async fn send_keysend(
		...
		// Generate a random preimage and compute its SHA256 hash for the payment.
		...
		Err(self.make_error("keysend stream ended without terminal status".to_string()))
	}
