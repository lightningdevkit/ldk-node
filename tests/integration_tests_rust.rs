...
use common::{
	...
	wait_for_block, wait_for_node_tip, wait_for_tx, InMemoryStore, NodePaymentExt, TestChainSource,
	TestConfig, TestNode, TestStoreType, TestSyncStore,
};
...
async fn onchain_send_receive() {
	...
	node_a.sync_wallets().unwrap();
	node_b.sync_wallets().unwrap();
	wait_for_node_tip(&node_a, new_height).await.expect("node hasn't sycned to the tip");
	wait_for_node_tip(&node_b, new_height).await.expect("node hasn't sycned to the tip");
	expect_channel_ready_event!(node_a, node_b.node_id());
	expect_channel_ready_event!(node_b, node_a.node_id());
...
async fn onchain_send_all_retains_reserve() {
	...
	node_a.sync_wallets().unwrap();
	node_b.sync_wallets().unwrap();
	wait_for_node_tip(&node_a, new_height).await.expect("node hasn't synced to the node tip");
	wait_for_node_tip(&node_b, new_height).await.expect("node hasn't synced to the node tip");
...
async fn onchain_wallet_recovery() {
	...
	original_node.sync_wallets().unwrap();
	wait_for_node_tip(&original_node, new_height)
		.await
		.expect("node hasn't synced to the node tip");
	assert_eq!(
	...
	let new_height = generate_blocks_and_wait(&bitcoind.client, &electrsd.client, 1).await;
	recovered_node.sync_wallets().unwrap();
	wait_for_node_tip(&recovered_node, new_height)
		.await
		.expect("node hasn't synced to the node tip");
	assert_eq!(
		recovered_node.list_balances().spendable_onchain_balance_sats,
...
async fn async_payment() {
	...
	node_receiver_lsp.sync_wallets().unwrap();
	node_receiver.sync_wallets().unwrap();
	wait_for_node_tip(&node_sender, new_height).await.expect("node hasn't synced to the node tip");
	wait_for_node_tip(&node_sender_lsp, new_height)
		.await
		.expect("node hasn't synced to the node tip");
	wait_for_node_tip(&node_receiver_lsp, new_height)
		.await
		.expect("node hasn't synced to the node tip");
	wait_for_node_tip(&node_receiver, new_height)
		.await
		.expect("node hasn't synced to the node tip");
...
async fn unified_send_receive_bip21_uri() {
	...
	// Make sure the transaction reached the mempool before mining, so the blocks include it.
	wait_for_tx(&electrsd.client, txid).await;
	let new_height = generate_blocks_and_wait(&bitcoind.client, &electrsd.client, 6).await;
...
async fn do_lsps2_client_service_integration(client_trusts_lsp: bool) {
	...
	service_node.sync_wallets().unwrap();
	payer_node.sync_wallets().unwrap();
	wait_for_node_tip(&service_node, new_height).await.expect("node hasn't synced to the tip");
	wait_for_node_tip(&payer_node, new_height).await.expect("node hasn't sycned to the tip");
...
async fn lsps2_client_trusts_lsp() {
	...
	client_node.sync_wallets().unwrap();
	payer_node.sync_wallets().unwrap();
	wait_for_node_tip(&service_node, new_height).await.expect("node hasn't sycned to the tip");
	wait_for_node_tip(&payer_node, new_height).await.expect("node hasn't sycned to the tip");
	assert_eq!(
		client_node
			.list_channels()
...
async fn lsps2_lsp_trusts_client_but_client_does_not_claim() {
	...
	client_node.sync_wallets().unwrap();
	wait_for_node_tip(&service_node, new_height).await.expect("node hasn't sycned to the tip");
	wait_for_node_tip(&client_node, new_height).await.expect("node hasn't sycned to the tip");
	assert_eq!(
		client_node
			.list_channels()
			.iter()
