...
use ldk_node::{BuildError, Builder, Event, NodeError, RecoveryMode};
...
async fn onchain_wallet_recovery() {
	...
	recovered_config.recovery_mode = Some(RecoveryMode::default());
...
async fn onchain_wallet_recovery_rescans_from_birthday_height() {
	// End-to-end test for `RecoveryMode { rescan_from_height }` against a bitcoind chain
	// source. The scenario:
	//
	...
	// 5. Restart a fresh node with just the seed (no recovery mode). Its wallet birthday
	...
	// 6. Restart again with `RecoveryMode { rescan_from_height: Some(birthday) }`. Now
	//    the wallet must find and report both funding transactions.
	let (bitcoind, electrsd) = setup_bitcoind_and_electrsd();
	...
	// Step 5: restart a fresh node with only the seed and no recovery mode. It must NOT see
	...
	assert_eq!(
		...
		0,
		"fresh node without recovery mode should not find funds below its wallet birthday"
	);
	pinned_node.stop().unwrap();
	drop(pinned_node);

	// Step 6: restart with recovery mode rescanning from the birthday height. Funds must be
	// re-discovered.
	let mut recovered_config = random_config(true);
	recovered_config.node_entropy = original_node_entropy;
	recovered_config.recovery_mode =
		Some(RecoveryMode { rescan_from_height: Some(birthday_height) });
	let recovered_node = setup_node(&chain_source, recovered_config);
	recovered_node.sync_wallets().unwrap();
	assert_eq!(
		recovered_node.list_balances().spendable_onchain_balance_sats,
