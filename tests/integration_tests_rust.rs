...
async fn start_stop_reinit() {
	...
	let test_sync_store =
		TestSyncStore::new(config.node_config.storage_dir_path.clone().into());

	let sync_config = EsploraSyncConfig { background_sync_config: None };
	setup_builder!(builder, config.node_config);
	builder.set_chain_source_esplora(esplora_url.clone(), Some(sync_config));

	let node = builder.build_with_store(test_sync_store.clone()).unwrap();
	node.start().unwrap();
	...
	let reinitialized_node = builder.build_with_store(test_sync_store).unwrap();
