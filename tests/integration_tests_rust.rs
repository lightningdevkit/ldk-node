...
async fn do_lsps2_client_service_integration(client_trusts_lsp: bool) {
	...
	service_builder.enable_liquidity_provider(Some(lsps2_service_config), None, false);
...
async fn do_lsps2_multi_lsp_picks_cheapest(reverse_order: bool) {
	...
	cheap_builder.enable_liquidity_provider(Some(cheap_cfg), None, false);
	...
	expensive_builder.enable_liquidity_provider(Some(expensive_cfg), None, false);
...
async fn lsps5_webhook_registration() {
	...
	service_builder.enable_liquidity_provider(None, Some(lsps5_service_config), false);
	let service_node = service_builder.build(service_config.node_entropy.into()).unwrap();
	service_node.start().unwrap();
	let service_node_id = service_node.node_id();
	let service_addr = service_node.onchain_payment().new_address().unwrap();
