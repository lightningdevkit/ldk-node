...
async fn lsps5_webhook_registration() {
	...
	service_builder.enable_liquidity_provider_lsps5(lsps5_service_config);
	let service_node = service_builder.build(service_config.node_entropy.into()).unwrap();
	service_node.start().unwrap();
	let service_node_id = service_node.node_id();
	let service_addr = service_node.onchain_payment().new_address().unwrap();
	...
	let results = lsps5_client
		.set_webhook(app_name_1.clone(), webhook_url_1.clone(), None)
		.expect("Failed to register webhook");
	assert_eq!(results.len(), 1, "Expected registration with the single configured LSP");
	...
	let results = lsps5_client
		.set_webhook(app_name_2.clone(), webhook_url_2.clone(), None)
	...
	let result = lsps5_client.set_webhook(app_name_3.clone(), webhook_url_3.clone(), None);
	assert_eq!(
		result.err(),
		Some(NodeError::LiquiditySetWebhookFailed),
		"Expected error when exceeding max webhooks"
	);

	// List registered webhooks
	let registered_webhooks = lsps5_client.list_webhooks(None).expect("Failed to list webhooks");
	...
	let result = lsps5_client.remove_webhook(non_existing_app_name.clone(), None);
	assert_eq!(
		result.err(),
		Some(NodeError::LiquidityRemoveWebhookFailed),
		"Expected error when removing non-existing webhook"
	);

	// Delete a registered webhook
	lsps5_client.remove_webhook(app_name_1.clone(), None).expect("Failed to delete first webhook");

	// Verify webhook was deleted
	let registered_webhooks =
		lsps5_client.list_webhooks(None).expect("Failed to list webhooks after deletion");
	...
	// Test service-side notification methods
