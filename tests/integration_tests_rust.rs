...
async fn bolt12_lsps2_client_service_integration() {
	...
	let fixed_offer = client_node
		.bolt12_payment()
		.receive_via_jit_channel(fixed_amount_msat, "fixed", None, None, None)
		.unwrap();
	assert!(fixed_offer.paths().is_empty());
	assert_eq!(fixed_offer.issuer_signing_pubkey(), Some(client_node.node_id()));
...
async fn do_lsps2_multi_lsp_picks_cheapest(reverse_order: bool) {
	...
	let offer = client
		.bolt12_payment()
		.receive_via_jit_channel(payment_amount_msat, "multi LSP", None, None, None)
		.unwrap();
