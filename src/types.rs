...
pub(crate) trait DynStoreTrait : Send + Sync {
	fn read_async(&self, primary_namespace: &str, secondary_namespace: &str, key: &str) -> Pin<Box<dyn Future<Output = Result<Vec<u8>, bitcoin::io::Error>> + Send + 'static>>;
	fn write_async(&self, primary_namespace: &str, secondary_namespace: &str, key: &str, buf: Vec<u8>) -> Pin<Box<dyn Future<Output = Result<(), bitcoin::io::Error>> + Send + 'static>>;
	fn remove_async(&self, primary_namespace: &str, secondary_namespace: &str, key: &str, lazy: bool) -> Pin<Box<dyn Future<Output = Result<(), bitcoin::io::Error>> + Send + 'static>>;
	fn list_async(&self, primary_namespace: &str, secondary_namespace: &str) -> Pin<Box<dyn Future<Output = Result<Vec<String>, bitcoin::io::Error>> + Send + 'static>>;

	fn read(&self, primary_namespace: &str, secondary_namespace: &str, key: &str) -> Result<Vec<u8>, bitcoin::io::Error>;
	fn write(&self, primary_namespace: &str, secondary_namespace: &str, key: &str, buf: Vec<u8>) -> Result<(), bitcoin::io::Error>;
	fn remove(&self, primary_namespace: &str, secondary_namespace: &str, key: &str, lazy: bool) -> Result<(), bitcoin::io::Error>;
...
impl<'a> KVStore for dyn DynStoreTrait + 'a {
	fn read(&self, primary_namespace: &str, secondary_namespace: &str, key: &str) -> impl Future<Output = Result<Vec<u8>, bitcoin::io::Error>> + Send + 'static {
		DynStoreTrait::read_async(self, primary_namespace, secondary_namespace, key)
	}

	fn write(&self, primary_namespace: &str, secondary_namespace: &str, key: &str, buf: Vec<u8>) -> impl Future<Output = Result<(), bitcoin::io::Error>> + Send + 'static {
		DynStoreTrait::write_async(self, primary_namespace, secondary_namespace, key, buf)
	}

	fn remove(&self, primary_namespace: &str, secondary_namespace: &str, key: &str, lazy: bool) -> impl Future<Output = Result<(), bitcoin::io::Error>> + Send + 'static {
		DynStoreTrait::remove_async(self, primary_namespace, secondary_namespace, key, lazy)
	}

	fn list(&self, primary_namespace: &str, secondary_namespace: &str) -> impl Future<Output = Result<Vec<String>, bitcoin::io::Error>> + Send + 'static {
