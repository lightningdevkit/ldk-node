...
use crate::payment::payjoin::payjoin_session::{
	PayjoinDirection, PayjoinSession, SerializedSessionEvent,
};
...
impl KVStorePayjoinReceiverPersister {
	...
	pub fn session_id(&self) -> PaymentId {
		self.session_id
	}

	pub async fn get_session(&self) -> Option<PayjoinSession> {
		self.payjoin_session_store.get(&self.session_id).await.ok()?
	}
	...
	pub async fn from_session(
		...
		if payjoin_session_store.get(&session_id).await?.is_none() {
	...
	pub async fn insert_inputs_seen(&self, inputs: Vec<OutPoint>) -> Result<(), Error> {
		...
		let mut session = self.get_session().await.ok_or(Error::InvalidPaymentId)?;
		for input in inputs {
