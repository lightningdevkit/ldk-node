...
impl PayjoinManager {
	...
	async fn handle_closed_session(
		...
		match outcome {
			...
			SessionOutcome::Cancel => {
				...
				self.close_session_with_fallback(&mut session);
			},
	...
	fn close_session_with_fallback(&self, session: &mut PayjoinSession) {
		session.status = PayjoinStatus::Failed;
		...
		if let Err(close_err) = self.payjoin_session_store.insert_or_update(session.clone()) {
			log_error!(
				self.logger,
