...
impl PayjoinManager {
		...
		match outcome {
			SessionOutcome::Success(_) => {
				...
				let kind = PaymentKind::Payjoin {
					txid: session.txid.ok_or(Error::InvalidPaymentId)?,
	...
	// TODO: we use the events returned from BDK wallet during
	// chain sync to know if the transaction is a payjoin tx
	//
	// Wallet sync can handle this, BUT `find_payment_by_txid` in wallet/mod.rs
	// needs to be updated to also search for PaymentKind::Payjoin by txid.
