...
pub struct PayjoinSession {
	...
	/// This is broadcast if the payjoin transaction fails.
	pub fallback_tx: Option<Transaction>,

	/// Inputs seen in this session's original proposal.
	///
	/// This is used to detect if the sender is reusing inputs across multiple sessions (only for receiver sessions).
...
impl PayjoinSession {
	pub fn new(
		...
		fee_rate_kwu: u64, fallback_tx: Option<Transaction>,
	) -> Self {
...
impl_writeable_tlv_based!(PayjoinSession, {
	...
	(6, fee_rate_kwu, required),
	(8, events, required_vec),
	(10, fallback_tx, option),
	(12, inputs_seen, optional_vec),
	(14, status, required),
	(16, completed_at, option),
	(18, latest_update_timestamp, (default_value, 0u64)),
});
...
impl UpdatableObject for PayjoinSession {
