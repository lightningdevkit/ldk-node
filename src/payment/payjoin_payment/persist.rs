...
use crate::payment::payjoin_payment::payjoin_session::{PayjoinStatus, SerializedSessionEvent};
use crate::Error;
use std::{
	sync::Arc,
...
use crate::payment::payjoin_payment::payjoin_session::{PayjoinDirection, PayjoinSession};
...
pub(crate) struct KVStorePayjoinSenderPersister {
...
impl KVStorePayjoinReceiverPersister {
	pub fn new(
		...
		let session = PayjoinSession::new(session_id, PayjoinDirection::Receive, None);
	...
	pub fn from_session(
		&self, session_id: PaymentId, kv_store: Arc<DynStore>, logger: Arc<Logger>,
	) -> Result<Self, Error> {
		let sessions = Self::load_all_sessions(&kv_store, &logger)?;
		let data_store = Arc::new(PayjoinSessionStore::new(
			sessions,
...
impl SessionPersister for KVStorePayjoinReceiverPersister {
