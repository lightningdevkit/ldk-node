...
impl Bolt12Payment {
	...
	fn connect_to_lsps2_peer(
		&self, liquidity_source: Arc<LiquiditySource<Arc<Logger>>>,
		...
		self.runtime.block_on(async move {
			connection_manager.connect_peer_if_necessary(con_node_id, con_addr).await
		})?;
	...
	fn receive_jit_channel_inner(
		...
		let (lsp_total_opening_fee, lsp_prop_opening_fee, payment_metadata) =
			self.runtime.block_on(async move {
				if let Some(amount_msat) = amount_msat {
					liquidity_source
						.lsps2_bolt12_payment_metadata(amount_msat, max_total_lsp_fee_limit_msat)
						.await
						.map(|(total_fee, payment_metadata)| {
							(Some(total_fee), None, payment_metadata)
						})
				} else {
					liquidity_source
	...
	fn receive_async_jit_channel_inner(
		...
		let peer_info = self.connect_to_lsps2_peer(Arc::clone(liquidity_source))?;
		let liquidity_source = Arc::clone(liquidity_source);
		let (lsp_prop_opening_fee, payment_metadata) = self.runtime.block_on(async move {
			liquidity_source
				.lsps2_variable_amount_bolt12_payment_metadata(
					max_proportional_lsp_fee_limit_ppm_msat,
				)
