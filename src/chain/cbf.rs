...
use crate::chain::{CbfFeeSourceConfig, ChainListener};
...
impl CbfChainSource {
	...
	pub(crate) fn start(&self, runtime: Arc<Runtime>, chain_listener: ChainListener) {
		...
		if let Some(onchain_wallet) = chain_listener.onchain_wallet.upgrade() {
			for script in onchain_wallet.list_revealed_scripts() {
...
fn resume_checkpoint(logger: &Logger, chain_listener: &ChainListener) -> Option<HashCheckpoint> {
	...
	let bdk_cp = chain_listener.onchain_wallet.upgrade()?.latest_checkpoint();
