...
impl Wallet {
	...
	pub(crate) fn get_full_scan_request(&self) -> FullScanRequest<KeychainKind> {
		self.inner.lck().start_full_scan().build()
	}

	pub(crate) fn get_incremental_sync_request(&self) -> SyncRequest<(KeychainKind, u32)> {
		self.inner.lck().start_sync_with_revealed_spks().build()
	}

	pub(crate) fn get_cached_txs(&self) -> Vec<Arc<Transaction>> {
		self.inner.lck().tx_graph().full_txs().map(|tx_node| tx_node.tx).collect()
	}
	...
	pub(crate) fn current_best_block(&self) -> BestBlock {
		let checkpoint = self.inner.lck().latest_checkpoint();
	...
	pub(crate) fn apply_update(&self, update: impl Into<Update>) -> Result<(), Error> {
		let mut locked_wallet = self.inner.lck();
		match locked_wallet.apply_update_events(update) {
			Ok(events) => {
				...
				let mut locked_persister = self.persister.lck();
	...
	pub(crate) fn get_balances(
		...
		let balance = self.inner.lck().balance();
	...
	fn list_confirmed_utxos_inner(&self) -> Result<Vec<Utxo>, ()> {
		let locked_wallet = self.inner.lck();
				...
				Err(CreateTxError::FeeRateTooLow { required: required_fee_rate }) => {
					if fee_rate.is_some() {
						log_error!(
							...
							fee_rate.expect("fee_rate.is_some() was checked above"),
							txid,
							required_fee_rate
						);
