	...
	L::Target: LdkLogger,
{
	...
	/// and across changes to our caching, and it is why an object updated mid-pagination cannot
	/// shift position and so be skipped or returned twice.
	///
	...
	async fn read_missing(
		...
		// Matches every backend's page size, so in practice this is a single wave. We keep refilling
		// anyway so that a backend paging more coarsely degrades gracefully rather than spawning an
		// unbounded number of tasks.
		const BATCH_SIZE: usize = 50;

		type ReadResult = (usize, String, Result<Vec<u8>, lightning::io::Error>);
		let spawn_read = |set: &mut tokio::task::JoinSet<ReadResult>, idx: usize, key: String| {
			let read_fut = KVStore::read(
				&*self.kv_store,
				&self.primary_namespace,
				&self.secondary_namespace,
				&key,
			);
			set.spawn(async move { (idx, key, read_fut.await) });
		};

		let mut missing = missing.into_iter();
		let mut set = tokio::task::JoinSet::new();
		while set.len() < BATCH_SIZE {
			let Some((idx, key)) = missing.next() else { break };
			spawn_read(&mut set, idx, key);
		}

		while let Some(join_res) = set.join_next().await {
			let (idx, key, read_res) = join_res.map_err(|e| {
				log_error!(self.logger, "Failed to join object read task: {}", e);
				set.abort_all();
				Error::PersistenceFailed
			})?;

			if let Some((next_idx, next_key)) = missing.next() {
				spawn_read(&mut set, next_idx, next_key);
			}
