		...
		match event {
			...
			LdkEvent::OpenChannelRequest {
				...
				if let Some(lsp2_node) = self
					.liquidity_source
					.as_ref()
					.and_then(|ls| ls.as_ref().get_lsp_config(&counterparty_node_id, Some(2)))
				{
					// If the peer is an LSPS2 node, additionally honor its trust_peer_0conf flag.
					allow_0conf = allow_0conf || lsp2_node.trust_peer_0conf;
