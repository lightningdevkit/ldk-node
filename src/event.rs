	...
	pub async fn handle_event(&self, event: LdkEvent) -> Result<(), ReplayEvent> {
		match event {
			...
			LdkEvent::ChannelClosed {
				...
				// every pending round the counterparty committed to, and our signatures may have
				// left the node for such a round, so it is kept (see `closed_channel_held_rounds`).
				// A payment left with no round of ours the monitor watches, and none LDK promoted
				// to the funding before, is failed: the monitor's `DiscardFunding` events settle
				// such payments once the close matures, but reach the handler ahead of this event
				// when one sync delivers the close and its maturity, and then find the channel
				// still listed with every round held. The monitor's guard is not `Send`, so its
