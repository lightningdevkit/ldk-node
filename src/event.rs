	...
	pub async fn handle_event(&self, event: LdkEvent) -> Result<(), ReplayEvent> {
		match event {
			...
			LdkEvent::PaymentForwarded {
				...
				self.channel_forwarding_stats_store.insert_or_update(inbound_stats).map_err(
					|e| {
						log_error!(
							self.logger,
				...
				self.channel_forwarding_stats_store.insert_or_update(outbound_stats).map_err(
					|e| {
						log_error!(
							self.logger,
				...
				match self.config.forwarded_payment_tracking_mode {
					...
					ForwardedPaymentTrackingMode::Stats => {
						// Do not store individual payment details
					},
				}
