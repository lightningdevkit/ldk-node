...
use crate::types::{DynStoreTrait, DynStoreWrapper, SyncAndAsyncKVStore};
...
// We do this to allow native Rust stores to satisfy the UniFFI-facing store trait
// in tests and internal bridge code. This keeps `DynStoreWrapper<T>` as the single
// adapter for Rust `KVStore`/`KVStoreSync` implementations while letting the UniFFI
// builder paths accept `Arc<dyn FfiDynStoreTrait>` without requiring a public
// `FfiDynStore` wrapper at call sites.
#[async_trait::async_trait]
