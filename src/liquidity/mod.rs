...
use crate::types::{
	Broadcaster, ChannelManager, DynStore, KeysManager, LSPS5ServiceConfig, LiquidityManager,
	Wallet,
};
	...
	L::Target: LdkLogger,
{
	...
	pub(crate) fn set_advertise_service(&mut self, advertise_service: bool) -> &mut Self {
	...
	pub(crate) async fn build(self) -> Result<LiquiditySource<L>, BuildError> {
		...
		Ok(LiquiditySource {
			...
			lsps5_service: Arc::new(LSPS5ServiceLiquiditySource {
				...
				peer_manager: RwLock::new(None),
				expiry_notified_at: Mutex::new(HashMap::new()),
