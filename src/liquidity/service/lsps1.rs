	...
	L::Target: LdkLogger,
{
	...
	pub(crate) async fn handle_event(&self, event: LSPS1ServiceEvent) {
		match event {
			LSPS1ServiceEvent::RequestForPaymentDetails {
					...
					&& lsps1_service_config.service_config.allow_onchain_payment
				{
					let address = self
						...
						.map_err(|e| {
							log_error!(
								self.logger,
								"Failed to generate LSPS1 Onchain Address: {:?}",
								e
							)
						})
						.expect("Should've gotten a valid address");

					onchain_addr_opt = Some(address.clone());

					payment_info.onchain = Some(LSPS1OnchainPaymentInfo {
						state: LSPS1PaymentState::ExpectPayment,
						expires_at: lsps1_service_config.service_config.payment_option_expires_at,
						fee_total_sat,
						order_total_sat,
						address,
						min_onchain_payment_confirmations: lsps1_service_config
							.service_config
							.min_onchain_payment_confirmations,
						min_fee_for_0conf: lsps1_service_config.service_config.min_fee_for_0conf,
						refund_onchain_address,
					});
				}

				if payment_hash_opt.is_none()
					&& offer_id_opt.is_none()
					&& onchain_addr_opt.is_none()
				{
					log_error!(
						self.logger,
				...
				if let Some(onchain_address) = onchain_addr_opt {
					if let Err(e) = self
						.kv_store
						.write(
							"lsps1_pending_orders",
							"",
							&onchain_address.script_pubkey().to_hex_string(),
							serialized_order.clone(),
						)
						.await
					{
						log_error!(
                            self.logger,
                            "Failed to persist pending LSPS1 order for Onchain Address {}: {:?}. Aborting request.",
                            onchain_address,
                            e
                        );
						return;
	...
	pub(crate) async fn process_lsps1_onchain_payments(&self, wallet: &Arc<Wallet>) {
		let unspent_utxos = match wallet.list_confirmed_utxos().await {
			Ok(utxos) => utxos,
			Err(e) => {
				log_error!(self.logger, "Failed to list unspent outputs for LSPS1 check: {:?}", e);
				return;
			},
		};

		for utxo in unspent_utxos {
			let script_hex = utxo.output.script_pubkey.to_hex_string();
			...
			if let Ok(bytes) = self.kv_store.read("lsps1_pending_orders", "", &script_hex).await {
				if let Ok(pending_order) = PendingLSPS1Order::read(&mut &bytes[..]) {
					...
					self.handle_order_payment_received(
					...
					if let Err(e) = self.channel_manager.create_channel(
						...
						log_error!(
							self.logger,
							"Failed to open LSPS1 channel after receiving funds: {:?}",
							e
						);
