...
use crate::liquidity::{
	is_lsps_node, select_lsps_for_protocol, LspNode, LIQUIDITY_REQUEST_TIMEOUT_SECS,
};
	...
	pub(crate) async fn handle_next_event(&self, event: LSPS1ClientEvent) {
		match event {
			LSPS1ClientEvent::SupportedOptionsReady {
				...
				if is_lsps_node(&self.lsp_nodes, &counterparty_node_id) {
...
impl LSPS1Liquidity {
	...
	pub fn request_channel(
		...
		let lsps1_node = select_lsps_for_protocol(&liquidity_source.lsp_nodes, 1, None)
			.ok_or(Error::LiquiditySourceUnavailable)?;
	...
	pub fn request_channel_from_lsp(
		...
		let lsps1_node = select_lsps_for_protocol(&liquidity_source.lsp_nodes, 1, Some(&node_id))
			.ok_or(Error::LiquiditySourceUnavailable)?;
