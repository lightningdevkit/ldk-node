	...
	L::Target: LdkLogger,
{
	...
	pub(crate) pending_set_webhook_requests:
		Mutex<HashMap<LSPSRequestId, PendingRequest<Result<SetWebhookResponse, LSPS5Error>>>>,
	pub(crate) pending_list_webhooks_requests:
		Mutex<HashMap<LSPSRequestId, PendingRequest<Result<ListWebhooksResponse, LSPS5Error>>>>,
	pub(crate) pending_remove_webhook_requests:
		Mutex<HashMap<LSPSRequestId, PendingRequest<Result<RemoveWebhookResponse, LSPS5Error>>>>,
	...
	L::Target: LdkLogger,
{
	pub(crate) async fn lsps5_set_webhook(
		...
		let _pending_request = {
			...
			PendingRequestGuard::insert(
				...
				&mut pending_set_webhook_requests_lock,
				request_id,
				sender,
			)
		};
	...
	pub(crate) async fn lsps5_list_webhooks(
		...
		let _pending_request = {
			...
			PendingRequestGuard::insert(
				...
				&mut pending_list_webhooks_requests_lock,
				request_id,
				sender,
			)
		};
	...
	pub(crate) async fn lsps5_remove_webhook(
		...
		let _pending_request = {
			...
			PendingRequestGuard::insert(
				...
				&mut pending_remove_webhook_requests_lock,
				request_id,
				sender,
			)
		};
	...
	fn deliver_response<T>(
		&self, pending: &Mutex<HashMap<LSPSRequestId, PendingRequest<T>>>,
		request_id: &LSPSRequestId, response: T,
	) {
		match pending.lock().expect("lock").remove(request_id) {
			Some(request) => {
				if request.sender.send(response).is_err() {
					log_error!(
						self.logger,
...
impl LSPS5Liquidity {
	...
	///
	/// Note that notifications are rate limited per client. Calling this repeatedly in quick
	/// succession returns [`Error::LiquidityNotifyWebhookRateLimited`], and the call may succeed
	/// once the LSP's cooldown period has elapsed.
