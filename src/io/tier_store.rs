...
mod tests {
	...
	fn setup_tier_store(ephemeral: bool, backup: bool) -> StorageFixture {
		...
		let primary: Arc<DynStore> =
			wrap_store!(Arc::new(FilesystemStore::new(base_dir.join("primary_store"))));
		let logger = Arc::new(
		...
		let ephemeral = if ephemeral {
			let eph_store: Arc<DynStore> =
				wrap_store!(Arc::new(FilesystemStore::new(base_dir.join("eph_store"))));
		...
		let backup = if backup {
			let backup: Arc<DynStore> =
				wrap_store!(Arc::new(FilesystemStore::new(base_dir.join("backup_store"))));
