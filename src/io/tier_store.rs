	...
	L::Target: LdkLogger,
{
	...
	pub(crate) fn snapshot(&self) -> Vec<((String, String, String), PendingBackupOp)> {
		self.pending.lock().unwrap().iter().map(|(k, v)| (k.clone(), v.clone())).collect()
	}
...
mod tests {
	...
	impl FailingStore {
		...
		fn should_fail_write(&self) -> bool {
			matches!(self.mode, FailMode::Write | FailMode::WriteAndRemove)
		}

		fn should_fail_remove(&self) -> bool {
			matches!(self.mode, FailMode::Remove | FailMode::WriteAndRemove)
		}
	}
	...
	fn read_retry_queue_ops(
		...
		PendingOpsDeserWrapper::read(&mut io::Cursor::new(data)).unwrap().0
	}
	...
	fn retry_task_replays_queued_write() {
		...
		queue.enqueue_sync(key.clone(), PendingBackupOp::Write { buf: data.clone() }).unwrap();
	...
	fn retry_task_replays_queued_remove() {
		...
		queue.enqueue_sync(key.clone(), PendingBackupOp::Remove { lazy: true }).unwrap();
	...
	fn retry_task_skips_stale_snapshot_entries() {
		...
		queue.enqueue_sync(key.clone(), PendingBackupOp::Write { buf: vec![1u8; 4] }).unwrap();

		let snapshot = queue.snapshot();

		queue.enqueue_sync(key.clone(), PendingBackupOp::Write { buf: vec![2u8; 4] }).unwrap();
	...
	fn pending_ops_roundtrip_ser_deser() {
		...
		let decoded = PendingOpsDeserWrapper::read(&mut io::Cursor::new(encoded)).unwrap().0;
	...
	fn retry_queue_dedups_write_then_write_to_latest() {
		...
		queue.enqueue_sync(key.clone(), PendingBackupOp::Write { buf: vec![1u8, 2u8] }).unwrap();
		queue.enqueue_sync(key.clone(), PendingBackupOp::Write { buf: vec![9u8, 8u8] }).unwrap();
