...
mod tests {
	...
	async fn test_queued_write_rechecks_closed_lock_connection() {
		...
		let write_task =
			tokio::spawn(KVStore::write(&store, "test_ns", "test_sub", "key", vec![1u8]));
		// Ensure the public method's initial check has passed and the internal write is queued on
		// the per-key lock before closing the store lock connection.
