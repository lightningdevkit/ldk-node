	...
	L::Target: LdkLogger,
{
		...
		for record in records {
			...
			let channel = match channel {
				...
				None => {
					...
					let _ = self.channel_record_store.remove(&user_channel_id).await;
					continue;
				},
			};
			...
			let _ = self
				.submit(&channel.channel_id, &counterparty_node_id, user_channel_id, intent)
				.await;
		}
	}
	...
	pub(crate) async fn on_negotiation_failed(
		...
		match reason {
			NegotiationFailureReason::PeerDisconnected => {
				...
				let _ =
					self.submit(&channel_id, &counterparty_node_id, user_channel_id, intent).await;
				false
			},
			...
			| NegotiationFailureReason::ContributionInvalid => {
					...
					.await
				{
					Ok(contribution) => {
						...
						let _ = self
							.submit(&channel_id, &counterparty_node_id, user_channel_id, intent)
							.await;
						false
					},
					Err(e) => {
	...
	async fn submit(
		...
		self.channel_record_store.update(update).await?;
